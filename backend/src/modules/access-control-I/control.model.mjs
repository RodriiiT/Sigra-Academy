...
export class UserModel {
    ...
    // Metodo para obtener todos los representantes
    static async getAllRepresentatives() {
        const [representatives] = await db.query(
            `SELECT * FROM representatives`
        );
        if (representatives.length === 0) return { error: 'No hay representantes registrados' };
        return {
    ...
    // Método para obtener un representante por su ID
    static async getRepresentativeById(representativeId) {
        if (!representativeId) return { error: 'El ID del representante es requerido' };
        const [representative] = await db.query(
            `SELECT * FROM representatives WHERE representative_id = ?`,
            [representativeId]
        );
        if (representative.length === 0) return { error: 'Representante no encontrado' };
        return {
