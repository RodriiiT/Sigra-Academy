...
export class ControlController {
    ...
    // Controlador para obtener todos los representantes
    getAllRepresentatives = async (req, res) => {
        try {
            const result = await this.model.getAllRepresentatives();
            if (result.error) return res.status(404).json({ error: result.error });
            return res.status(200).json({
                message: result.message,
    ...
    // Controlador para obtener un representante por su ID
    getRepresentativeById = async (req, res) => {
        const { representativeId } = req.params;
        try {
            const result = await this.model.getRepresentativeById(representativeId);
            if (result.error) return res.status(404).json({ error: result.error });
            return res.status(200).json({
                message: result.message,
